  ...
  highWaterMark(64*1024*1024)
{
  ...
  p_channel->setWriteCallBack(std::bind(&TcpConnection::handleWrite, this));
  p_channel->setCloseCallBack(std::bind(&TcpConnection::handleClose, this));
  p_channel->setErrorCallBack(std::bind(&TcpConnection::handleError, this));
...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
  ...
  if(!faultError && remaining > 0)
  {
    ...
    {
      p_loop->queueInLoop(std::bind(m_highWaterMarkCallback, shared_from_this(),
                                    oldLen + remaining));
    }
...
void TcpConnection::handleRead()
{
  ...
  if(n > 0){
    m_messageCallBack(shared_from_this(), &m_inputBuffer, TimeStamp::now());
  }
  else if ( n==0 ){
    handleClose();
  }
...
void TcpConnection::handleWrite()
{
  ...
  if(p_channel->isWriting())
  {
    ssize_t n = sockets::write(p_channel->fd(), 
                               m_outputBuffer.peek(), 
